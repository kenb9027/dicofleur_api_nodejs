...
exports.findAllUser = (req, res) => {
    ...
    console.log(`${req.method} ${req.originalUrl}, Fetching users.`);
    User.findAll({
        ...
        .catch((err) => {
            res.status(HttpStatus.INTERNAL_SERVER_ERROR.code).send(
                new Response(
                    ...
                    `Some error occurred while retrieving the accounts.`
                )
            );
        });
};
        ...
        .catch((err) => {
            res.status(HttpStatus.INTERNAL_SERVER_ERROR.code).send(
                new Response(
                    HttpStatus.INTERNAL_SERVER_ERROR.code,
                    HttpStatus.INTERNAL_SERVER_ERROR.message,
                    `Error retrieving account with id = ${id} `
                )
            );
        });
};
...
//     console.log(`${req.method} ${req.originalUrl}, Updating user password.`);
